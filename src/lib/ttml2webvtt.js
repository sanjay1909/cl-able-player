...
function computeEndTime(startTime, durationTime) {
  var SECONDS = 0;
  var MINUTES = 1;
  var HOURS = 2;
...
export default function ttml2webvtt(contents) {
  var xml = convert.xml2json(contents, {
    ignoreComment: true,
    alwaysChildren: true,
    compact: true,
    spaces: 2
  ...
  var vttCaptions = captions.reduce(function(acc, value, index) {
    ...
    var endTime = computeEndTime(attributes.begin, attributes.dur);

    var caption =
    computeEndTime(attributes.begin, '00:00:0') +
    ' --> ' +
    computeEndTime(attributes.begin, attributes.dur) +
    '\n' +
    (isArray ? text.join('\n') : text) +
    '\n\n';
